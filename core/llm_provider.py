  ...
  "cards": [
    {
      ...
      "updated_summary": "实现用户登录功能，编写单元测试，并测试功能的正确性"
    }
  ]
}

6. 合并卡片（merge_with_previous）：
   - 如果当前卡片的活动与上一张卡片完全相同或高度相似，设置 merge_with_previous=true
   - 系统会自动将当前卡片合并到上一张卡片中
   - 合并时，上一张卡片的结束时间会延长到当前卡片的结束时间
   - 如果选择合并卡片
      - 提供 summary 时，程序会将 summary 内容追加到上一张卡片的 summary 中
      - 如果卡片不断追加导致冗长，可以提供 updated_summary 来覆盖上一张卡片的 summary 内容，此时程序将忽略 summary 内容
      - 优先使用 updated_summary 内容对卡片内容进行更新，而不是 summary 内容
...
class DayflowBackendProvider:
    ...
    async def generate_activity_cards(
        ...
        if context_cards:
            ...
            if start_time and prev_card._next_card_start_time:
                ...
                if time_gap > 60:
                    obs_text += "⚠️ 时间间隔较长时，必须创建新卡片，禁止合并！\n"
    ...
    def _parse_json_from_text(text: str) -> Optional[dict | list]:
        ...
        3. 修复常见格式问题的JSON（多余逗号、注释等）
        
        Args:
            text: AI返回的文本，可能包含JSON数据
            
        Returns:
            解析成功的JSON对象或数组，解析失败返回None
        """
        if not text:
            return None
        ...
        # 方法0：移除markdown代码块标记（```json ... ``` 或 ``` ... ```）
        markdown_match = re.search(r'```(?:json)?\s*([\s\S]*?)```', cleaned_text)
        if markdown_match:
        ...
        if json_data is None:
            try:
                ...
                # 再次尝试提取JSON
                json_match = re.search(r'\{[\s\S]*\}|\[[\s\S]*\]', fixed_text)
                if json_match:
                    json_data = json.loads(json_match.group())
                    parse_method_used = "方法3-修复后的JSON"
            except json.JSONDecodeError as e:
                logger.debug(f"方法3解析失败: {e}")
        
        # 方法4：直接尝试解析整个文本（如果文本本身就是JSON）
        if json_data is None:
            try:
                json_data = json.loads(cleaned_text)
                parse_method_used = "方法4-直接解析"
            except json.JSONDecodeError as e:
                logger.debug(f"方法4解析失败: {e}")
        
        if json_data is not None:
            logger.debug(f"使用{parse_method_used}成功解析JSON")
        else:
            logger.warning(f"无法从响应中提取有效的JSON数据")
            logger.info(f"原始响应内容（前500字符）:\n{text[:500]}")
    ...
    def _parse_observations_from_text(self, text: str, duration: float) -> List[Observation]:
        ...
        try:
            ...
            logger.warning(f"解析观察记录时出错: {e}, 原文: {text[:200]}")
            # 如果解析失败，创建一个基于整段文本的观察记录
            observations.append(Observation(
                start_ts=0,
                end_ts=duration,
                text=text[:500]
    ...
    def _parse_cards_from_text(
        ...
        try:
            # 使用统一的JSON解析方法
            json_data = self._parse_json_from_text(text)
            if json_data is None:
                return []
